...
$item = query("SELECT `user_transaksi`.*, `item_market`.*
FROM `user_transaksi` JOIN `item_market`
ON `item_market`.`id_item` = `user_transaksi`.`id_item` WHERE `user_transaksi`.`id_transaksi` = '$id_transaksi'
")[0];

if (isset($_POST["bukti"])) {
    if (bukti($_POST) > 0) {
        header("Location: user-history-delivery.php");
        $_SESSION["bukti_transaksi"] = true;
    } else {
        ...
        <div class="container">
            <div class="row justify-content-center">
                ...
                <div class="col-lg-3 mr-3">
                    ...
                    <p class="mb-3"><?= $item['item_type']; ?> | Beli : <?= $item['many_transaksi']; ?> Barang </p>
                    <a href="">Total Rp. <?= $item['price_transaksi']; ?> </a>
                ...
                <div class="col-lg-3 info-tmbh  justify-content-end">
                    ...
                    <div class="form-group">
                        <textarea class="form-control mt-2" id="exampleFormControlTextarea1" rows="3" cols="1" name="info" readonly><?= $item['info_transaksi'] ?></textarea>
                    </div>
                    <form action="" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="id_transaksi" value="<?= $item['id_transaksi']; ?>">
                        <p class="mb-1">Upload bukti pembayaran : </p>
                        <div class="custom-file">
                            <input type="file" class="custom-file-input" id="customFile" name="bukti_transaksi">
                            <label class="custom-file-label" for="customFile">Pilih
                                Gambar</label>
                        </div>
                        <button type="submit" name="bukti" class="mt-2">Upload</button>
                    </form>
                </div>
            </div>
        </div>
